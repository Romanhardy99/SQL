...
-- CAST(value AS TYPE); Явное преобразования значения (value) в желаемый тип (TYPE); (type)value;
...
-- DATEADD(UINT, amount, date OR time); Добавляет заданное количество (amount) едениц времени (UINT) к дате либо времени (date OR time);
 --IIF(CONDITION, value_1, value_2); Conditional Ternary Operator: Если условие (CONDITION) вернуло TRUE, то IIF возвращает значение 'value_1', в противном случае - 'value_2';
